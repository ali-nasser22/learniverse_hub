...
export default async function EnrolledCourseCard({
    ...
    // Progress Bar logic
    const courseDetails = await getCourseById(course?._id.toString());
    const totalModulesCount = courseDetails?.modules?.length;
    const totalCompletedModules = report?.totalCompletedModules?.length;
    const progressPercentage = (totalCompletedModules! / totalModulesCount!) * 100;
    return (
        <div
            key={course.id}
        ...
        >
          ...
          {category?.title ?? "No Category"}
            ...
            <span className="text-md md:text-sm font-medium text-slate-700">
              Total Quizzes: {report?.quizAssessment?.assessments?.length ?? 0}
                        ...
                        <div className="text-md md:text-sm font-medium text-slate-700">
                            Quiz taken{" "}
                            <Badge variant="outline">{attemptedQuiz?.length ?? 0}</Badge>
                        </div>
                    </div>
                    <div className="flex items-center justify-between mt-2">
            <span className="text-md md:text-sm font-medium text-slate-700">
            ...
            <span className="text-md md:text-sm font-medium text-slate-700">
              Other Marks
                        ...
                        <span className="text-md md:text-sm font-medium text-slate-700">
              {report?.quizAssessment?.otherMarks ?? 0}
            </span>
                    </div>
                </div>
          ...
          <span className="text-md md:text-sm font-medium text-slate-700">
            ...
            {mark + (report?.quizAssessment?.otherMarks ?? 0)}
                ...
                <CourseProgress size="sm" value={progressPercentage || 0}
